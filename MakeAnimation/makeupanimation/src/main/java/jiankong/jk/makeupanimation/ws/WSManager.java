...
public class WSManager implements IWManager {
    ...
    private Handler wshandler=new Handler(Looper.myLooper());
    private int reContionCount=0;
    private Runnable reConntionRunable=new Runnable() {
        @Override
        public void run() {
    ...
    private synchronized void buildeConn(){
    ...
    private void tryReconnect(){
       ...
       // long dely=reContionCount*RECONNECT_INTERVAL;
        wshandler.postDelayed(reConntionRunable,10000);
        reContionCount++;
    }
    private void cancelReConntion(){
        wshandler.removeCallbacks(reConntionRunable);
        reContionCount=0;
    }
    ...
    private void disConnect(){
        ...
        if (mWebSocket!=null){
            boolean isClose=mWebSocket.close(WsStatus.DISCONNECTED,WsStatus.TIP.NORMAL_CLOSE);
            if (!isClose){
